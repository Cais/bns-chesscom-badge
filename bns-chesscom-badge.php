...
class BNS_Chesscom_Badge_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => esc_html__( 'Displays a Chess.com member badge in a widget area; or, with a shortcode.', 'bns-chesscom-badge' ),
		);
	...
	function widget( $args, $instance ) {
		...
		/** The user name trimmed of any white space. */
		...
		/** Base URL of API to access user details. */
		$the_source = 'https://www.chess.com/api/get_user_info?username=';
		/** Full API URL of user details. */
		...
		switch ( $badge ) {
			case '125x125':
				?>
				<a href="http://www.chess.com?ref_id=<?php echo esc_attr__( $user_id ); ?>">
			...
			case '250x250':
				?>
				<a href="http://www.chess.com?ref_id=<?php esc_attr__( $user_id ); ?>">
				...
				<div style="border: 1px solid #000; width: 170px; overflow: hidden; font-family: Verdana, Arial, sans-serif; margin: 4px auto;">
					<div style="border: 2px solid #9ac567; border-right-color: #224d00; border-bottom-color: #224d00; background-color: #4a7521; font-size: 12px;">
						<div style="margin: 4px; background-color: #fff; padding: 4px; text-align: center; border: 1px solid #9ac567;">
							<a href="http://www.chess.com?ref_id=<?php echo esc_attr__( $user_id ); ?>" rel="nofollow" style="font-size: 9px;">
								<img src="http://cssjs.chesscomfiles.com/images/chesscom_logo.gif" alt="" border="0" />
							</a>
							<a href="http://www.chess.com?ref_id=<?php echo esc_attr__( $user_id ); ?>" rel="nofollow" style="font-size: 9px;"></a>
						</div>
						...
						<div style="margin: 4px; background-color: #fff; padding: 6px; border: 1px solid #9ac567;">
							<a href="http://www.chess.com/members/view/Cais?ref_id=<?php echo esc_attr__( $user_id ); ?>" rel="nofollow"><img src="<?php echo $avatar_url; ?>" width="30" height="30" alt="" style="float: left; margin: 0 4px 4px 0; border: 1px solid #666;" /></a><a href="http://www.chess.com/members/view/<?php echo esc_attr__( $username ); ?>?ref_id=<?php echo $user_id; ?>" rel="nofollow"><strong><?php echo esc_attr__( $username ); ?></strong></a><br />Rating:
							<strong><?php echo $best_rating; ?></strong><br /><br />

							<div style="text-align:center; margin: 8px;">
								<a href="http://www.chess.com/echess/create_game.html?uid=<?php echo esc_attr__( $user_id ); ?>&ref_id=<?php echo esc_attr__( $user_id ); ?>" style="border: 2px solid #f7b15b; border-right-color: #db8213; border-bottom-color: #db8213; background-color: #ff9c21; color: #ffc; padding: 1px 2px; text-decoration: none; font-weight: bold; font-size: 14px;" rel="nofollow">Challenge me!</a>
							</div>
							<div style="text-align:center;">
								<a href="http://www.chess.com/home/game_archive.html?member=<?php echo esc_attr__( $username ); ?>&ref_id=<?php echo esc_attr__( $user_id ); ?>" rel="nofollow">View my games</a>
							</div>
						</div>
					</div>
				</div>
		...
		if ( ( 'online' === $online_status_image_url ) && ( true === $instance['status'] ) ) {
			echo apply_filters( 'bnscb_online_text', sprintf( '<div class="bnscb_online bnscb_status">%1$s</div>', esc_html__( 'I am online and ready to play!', 'bns-chesscom-badge' ) ) );
		} elseif ( ( 'offline' === $online_status_image_url ) && ( true === $instance['status'] ) ) {
			echo apply_filters( 'bnscb_online_text', sprintf( '<div class="bnscb_offline bnscb_status">%1$s</div>', esc_html__( 'I am offline but accepting challenges!', 'bns-chesscom-badge' ) ) );
		}
	 ...
	 * @since   0.1
	 *
	 * @param   array $new_instance new widget parameters.
	 * @param   array $old_instance old widget parameters.
	 *
	 * @return  array
	 */
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'bns-chesscom-badge' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'the_user' ) ); ?>"><?php esc_html_e( 'Enter your Chess.com user name:', 'bns-chesscom-badge' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'the_user' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'the_user' ) ); ?>" value="<?php echo esc_attr( $instance['the_user'] ); ?>" style="width:100%;" />
		</p>

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'badge' ) ); ?>"><?php esc_html_e( 'Choose Badge Size:', 'bns-chesscom-badge' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'badge' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'badge' ) ); ?>" class="widefat">
		...
		<p><!-- This (checkbox) is used to turn on or off if the message is displayed -->
			<input class="checkbox" type="checkbox" <?php checked( (bool) $instance['status'], true ); ?> id="<?php echo esc_attr( $this->get_field_id( 'status' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'status' ) ); ?>" />
			<label for="<?php echo esc_attr( $this->get_field_id( 'status' ) ); ?>"><?php esc_html_e( 'Show your online status?', 'bns-chesscom-badge' ); ?></label>
		</p>
...
/** Instantiate the extended widget class */
 ...
 * @see     wp_remote_get
 *
 * @param array $args existing values.
 */
